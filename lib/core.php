...
/* отсюда из ядра используются остальные модули, 
 * в них идет проверка на правильность использования
 * потому объявляется переменная
 * TODO: Может на свою переделать
 */
define("CMS", "CMS");
...
/*
 * проверка на установленость переменной
 */
function set(&$var) {
    return empty($var) && $var !== "" && $var !== false ? false : true;
}
...
function cmsErrorHandler($errno, $errmsg, $filename, $linenum, $vars) {
    ...
    if (striStr($errmsg, "Use of undefined constant") || striStr($errmsg, "Undefined index") || striStr($errmsg, "Uninitialized string offset") || striStr($errmsg, "Undefined offset"))
        $type = false;
...
if (!set($_SERVER ["errors"]))
    $_SERVER ["errors"] = false;
if (!set($_SERVER ["lang"]))
    $_SERVER ["lang"] = "ru";
...
$_ENV ["TMP"] = ($_ENV ["TMP"]) ? $_ENV ["TMP"] : $_SERVER ["DOCUMENT_ROOT"] . "/tmp";
...
// Контрибуты
if (count($_SERVER["contrib"]))
    foreach ($_SERVER["contrib"] as $path => $name) {
...
$autoexecStuff[] = $_SERVER["CORE"] . "/classes/form_ajax/form_ajax_class.php";
$autoexecStuff[] = $_SERVER["CORE"] . "/classes/form_ajax/form_ajax_lang.php";
//$autoexecStuff[] = $_SERVER["CORE"] . "/classes/mail.php";
// Модули
if (count($_SERVER["modules"]))
    foreach ($_SERVER["modules"] as $path => $name) {

        $file = $_SERVER["DOCUMENT_ROOT"] . "/lib/modules/{$path}/includes/{$path}_autoexec.php";
        if (file_exists($file))
            $autoexecStuff[] = $file;

        $file = $_SERVER["DOCUMENT_ROOT"] . "/lib/modules/{$path}/includes/{$path}_lang.php";
        if (file_exists($file))
            $autoexecStuff[] = $file;
    }
...
if ($_SERVER["debug"]["noCache"]["php"]) { // not making cache
    ...
    if ($_SERVER["modules"]["cache"])
        require($_SERVER["DOCUMENT_ROOT"] . "/lib/modules/cache/includes/cache_get.php");
