<? 
	...
	function cmsFile_temp($text, $prefix='tmp') {
		...
		} else { print cmsError("Невозможно записать временный файл «{$path}»."); exit; }
		
		if (!touch($path)) { print cmsError("Невозможно записать временный файл «{$path}»."); exit; }
	...
	function cmsFile_tempClear() {
		...
		if (is_array($_SERVER[tempFiles])) foreach ($_SERVER[tempFiles] as $tmp) unlink($tmp);
	...
	function cmsFile_download($url, $report=false) {
		...
		if (mb_substr($url, 0, mb_strlen($local))) $url = "http://" . $_SERVER['HTTP_HOST'] . mb_substr($url, mb_strlen($local));
	...
	function cmsFile_list($path, $recursion = true, $level = 0) {
		...
		if (@is_dir($path)) {
			...
			while (($e = readdir($d)) !== false) {
				...
				if ($e != '.' && $e != '..' && ($recursion || $level == 1)) $array = array_merge(cmsFile_list($path . '/' . $e, $recursion, $level), $array);
	...
	*/
	...
	function cmsFile_delete($dir) {
		...
		if(!$dh = @opendir($dir)) return $list;
		
		while (false !== ($obj = readdir($dh))) {
			
			if($obj == '.' || $obj == '..') continue;
			if (!@unlink($dir . '/' . $obj)) $list = array_merge($list, cmsFile_delete($dir . '/' . $obj)); else $list[] = $dir . '/' . $obj;
	...
	function cmsFile_path($file) { // no matter
		...
		if (mb_substr($file, 0, $rootLen) == $root) return $file; //already absolute
		elseif (mb_substr($file, 0, 1) == "/") return $root . $file;
	...
	function cmsFile_touch($file, $dir=false) { // no matter
		...
		if ($dir) return is_dir($file); else return is_file($file);
	...
	function cmsFile_size($file, $size='KB') { // no matter
		...
		if ($size == 'KB') { $q = 1024; $size = 'КБ'; }
		elseif ($size == 'MB') { $q = 1024 * 1024; $size = 'МБ'; }
		else { $q = 1; $size = 'Б'; }
	...
	function cmsFile_info($file) {
		...
		$return = array(
			"path"		=> $info[dirname],
			"pathRel"	=> cmsFile_pathRel($info[dirname]),
			"file"		=> $file,
			"fileRel"	=> cmsFile_pathRel($file),
			"name"		=> basename($file, ".{$info[extension]}"),
			"full"		=> basename($file),
			"ext"			=> $info[extension],
			"size"		=> cmsFile_size($file),
			"date"		=> cmsFile_date($file, $_SERVER[lang]),
			"intDate"	=> @filemtime($file),
		);
