	...
	var cmsForm_ajax = {
		...
		submit: function(formObj) {
			...
			// не будем использовать пока форму в оригинальном смысле
			return false;
		...
		/**
		 *	Перезагружает CAPTCHA, возможный TO_DO — отлов всех капч для formID, но по-идее форма с капчей должна быть одна на страницу
		 *	@param	string		formName		Название формы
		 *	@return	void
		 */
		reloadCaptcha: function(formName, formID) {
			...
			img.attr("src", "/core/classes/form_ajax/ajax_confirm.php?formName=" + formName + "&width=" + img.width() + "&height=" + img.height() + "&rnd=" + Math.random());
