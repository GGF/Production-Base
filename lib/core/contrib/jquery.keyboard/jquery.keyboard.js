			...
			rm : function (e) {
				for (var i = 0; i < $k.keys.cont.length; i++) {
					if ($k.keys.cont[i].keyCode == e.keyCode) {
						$k.keys.cont.splice(i,1); // delete тут не проходит изза невозможности перевести в объект
						$k.keys.dump('Remove ');
						return;
					}
				}
		...
		bind : function ($obj, args) {
			...
			if (args.setup) {
				...
				if (!uid | !$k.keyboardBinds[uid]) { // если нет - добавим
					...
					$obj.keydown(function (e) {$k.checkBinds($obj, e);});
					$obj.keyup(function (e) {$k.checkBinds($obj, e);});
