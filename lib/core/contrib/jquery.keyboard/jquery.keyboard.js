(function ($) {	
	...
	var $k = {
		...
		getRange : function (title) {
			...
			switch (title) {
				...
				case 'printable': return f('letters').concat(
				                         f('allnum') .concat(
				                         f('symbols')))
				default         : throw 'No such range: В«' + title + 'В»';
			}
		},
		...
		checkBinds : function ($obj, e) {
			...
			var ei, okb = $k.keyboardBinds[uid][e.originalEvent.type]; // окб по типу события
			...
			debug(e.originalEvent.type+' => '+e.originalEvent.keyCode+' '+okb.length);
			for (var i in okb) { // Экономия отработки - не проходит если приязано на отпускание а событие нажатие
				var bind = okb[i];
				//if (bind.cfg.event == e.originalEvent.type) { // это уже прошло по второму индексу массива
					ei = $k.match(bind);
					if ( ei && $k.hasCurrent(bind, e) ) {
						//var backup = $obj.keyboardFunc; // объект передается по значению и при выходе из функции будет как был - нет смысла сохранять
						var events = [];
						for (var k in ei) {
						...
						//$obj.keyboardFunc = backup; // смотри выше
		...
		bind : function ($obj, args) {
			...
			if (args.setup) {
				...
				if (!uid | !$k.keyboardBinds[uid]) { // если нет - добавим
					var date = new Date();
					uid = date.getTime(); // uniqid - уникальный индекс в массиве
				...
				if (!$k.keyboardBinds[uid][args.cfg.event]) { // тут проверяем на привязку к событию, это сделано чтоб не гонять на оба события если не нужно
					$k.keyboardBinds[uid][args.cfg.event] = {};
					$obj.bind(args.cfg.event,function (e) {
							$k.checkBinds($obj, e);
						});
	...
	var empty = function (mixed_var) { // проверка на пустоту переменнтой - универсальная
		var key;
